...
<head>
<meta charset="utf-8">
<title>Task Creation Results</title>
				...
				<tbody>
					<%
						try {
							...
							ResultSet rs = stmt.executeQuery(
									"select task.title as title, task.description as description, priority, status, story.title as sname, taskId, userId from task inner join story on task.storyId = story.storyId;");
							while (rs.next()) {
					%>
					<%
					...
					<tr>
						...
						<td><select type="text" class="form-control" name="status"
							onChange="statusChanged(this.options[this.selectedIndex].value, <%=rs.getInt("taskId")%>)">
								<option value="<%=defaultStatus%>" selected><%=defaultStatus%></option>
								<option value="<%=status1%>"><%=status1%></option>
								<option value="<%=status2%>"><%=status2%></option>
						</select></td>
						<td><select type="text" class="form-control" name="assign"
							onChange="assignTo(this.options[this.selectedIndex].value,<%=rs.getInt("taskId")%>)">
								<%
